...
public class UserDAO {
     ...
     *
     * @param user The user object with plain text password to be hashed
     * @param hashPassword Whether to hash the password before storing
     * @return The generated user ID if successful
     * @throws SQLException If a database access error occurs
     */
    public int addUser(User user, boolean hashPassword) throws SQLException {
        try (Connection conn = DatabaseConnection.getConnection();
             PreparedStatement ps = conn.prepareStatement(INSERT_USER_SQL, Statement.RETURN_GENERATED_KEYS)) {
    ...
    /**
     * Updates a user with a hashed password
     *
     * @param user The user to update, with password already hashed
    ...
    public boolean updateUser(User user) {
        try (Connection connection = DatabaseConnection.getConnection();
            ...
            ps.setString(1, user.getFullName());
            ps.setString(2, user.getEmail());
            ps.setString(3, user.getPassword()); // Password should already be hashed before calling this method
            ps.setString(4, user.getAccountType().name().toLowerCase());
            ps.setString(5, user.getAddress());
            ps.setString(6, user.getPhoneNumber());
